...
instance ( KnownNat bytes, Message body ) => Message (WithKademliaIdentifier bytes body) where
    messageName _ =
        let MessageName sub = messageName (Proxy :: Proxy body) 
        in  MessageName $ BS.append sub "_kb"

    formatMessage = formatMessage . wkiBody
...
kademliaBroadcast kd = do
    initiator <- pure $ kademliaInitiator kd
    withRepeater <- pure $ kademliaWithRepeater kd
...
eliminateId k = filter ((/=) k . K.nodeId)
...
bucketsCloserTo here there tree@(KT.NodeTree _ _) =
    ...
    trimBucket bshere bsthere = filter $ \node ->
        let fromHere = zipWith xor (toByteStruct (K.nodeId node)) bshere
            toThere = zipWith xor (toByteStruct (K.nodeId node)) bsthere
    ...
    trimBucket'
        ...
        -> [Node (KIdentifier bytes)]
        -> [Node (KIdentifier bytes)]
    trimBucket' here there = filter $ \node -> go here there (toByteStruct (K.nodeId node))
      where
...
organizeBucket kId = M.elems . msbMap
    ...
    msbMap = flip foldr M.empty $ \node ->
        M.alter (maybe (Just [node]) (Just . (:) node)) (MostSignificant (K.nodeId node `distance` kId))
