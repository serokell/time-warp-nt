...
import           Mockable.Concurrent         (Async, Concurrently, Delay, delay,
                                              forConcurrently, fork, wait, withAsync)
...
timeout str us m = do
    ...
    withAsync action $ \__actionPromise -> do
        withAsync timeoutAction $ \__timeoutPromise -> do
...
sendAll ConversationStyle sendActions peerId msgs =
    timeout "sendAll" 30000000 $
        void . withConnectionTo sendActions peerId $ \__peerData ->
...
deliveryTest transport_ testState workers listeners = runProduction $ do
    ...
    let server = node (simpleNodeEndPoint transport) (const noReceiveDelay) prng1 BinaryP runBinaryP () defaultNodeEnvironment $ \serverNode -> do
    ...
    let client = node (simpleNodeEndPoint transport) (const noReceiveDelay) prng2 BinaryP runBinaryP () defaultNodeEnvironment $ \clientNode ->
