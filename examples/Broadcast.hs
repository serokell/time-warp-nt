...
makeListener transport sharedState i k = do
    ...
    node (simpleNodeEndPoint transport) (const noReceiveDelay) prng BinaryP $ \node -> do
        ...
        bcast <- kademliaBroadcast discovery
...
makeBroadcaster transport n = do
    ...
    node (simpleNodeEndPoint transport) (const noReceiveDelay) prng1 BinaryP $ \node -> do
        let localAddress = nodeEndPointAddress node
