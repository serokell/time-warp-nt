...
main = runProduction $ do
    ...
    node transport prng1 rateLimitingUnbounded BinaryP (B8.pack "I am node 1") defaultNodeEnvironment $ \node1 ->
        NodeAction (listeners . nodeId $ node1) $ \sactions1 -> do
            _ <- setupMonitor 8000 runProduction node1
            node transport prng2 rateLimitingUnbounded BinaryP (B8.pack "I am node 2") defaultNodeEnvironment $ \node2 ->
