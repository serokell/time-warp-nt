...
listeners :: Applicative m
          => NodeId
          -> BS.ByteString
          -> m [Listener Packing BS.ByteString Production]
listeners anId = const $ pure [pongListener]
    where
...
main = runProduction $ do
    ...
    node (simpleNodeEndPoint transport) (const noReceiveDelay) prng1 BinaryP runBinaryP (B8.pack "I am node 1") defaultNodeEnvironment $ \node1 ->
        NodeAction (listeners . nodeId $ node1) $ \sactions1 -> do
            _ <- startMonitor 8000 runProduction node1
            node (simpleNodeEndPoint transport) (const noReceiveDelay) prng2 BinaryP runBinaryP (B8.pack "I am node 2") defaultNodeEnvironment $ \node2 ->
